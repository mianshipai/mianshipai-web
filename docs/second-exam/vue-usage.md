...
在 Vue 组件中，data 选项必须是一个函数而不是一个对象，这是 Vue 的设计决策，主要基于以下原因：

::: details 参考答案

### 避免数据共享问题

- 对象引用问题：如果 data 是一个对象，所有组件实例将共享同一个数据对象。当一个组件修改数据时，会影响其他所有实例的数据状态。

- 函数隔离作用域：通过函数返回新对象，确保每个组件实例都能获得独立的数据副本。
...
💡 根本原因：保证组件复用时每个实例都有独立的数据状态，避免数据污染

💡 特殊场景：根实例可以使用对象形式

💡 最佳实践：始终使用函数返回对象的形式

关联文章：[Vue组件的data是一个对象还是函数？为什么？！](https://juejin.cn/post/7515390154309697545)
